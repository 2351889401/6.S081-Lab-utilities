...
### （这是个人的浅浅见解，不一定对。但是我觉得如果真的可以把困难的技术、简化到普通人都可以很轻松的使用，这本身对技术推动就是很了不起的一件事。比如抖音，虽然是短视频平台，但是它就是做到了让老百姓很轻松的去完成拍摄、发布，这些本来要求相对较高的事情，所以它能获得很大的流行。这也是科技真正带给大家幸福的见证。我也希望所有的科技发展，最终都能给人们带去幸福！）
...
而直到左子进程完成，成功退出（执行了**exit**，此时会释放所有的文件描述符），这时管道的写端口文件描述符释放，读端口才能变成非阻塞的状态，右子进程才能继续执行下去，这样保证了管道命令的顺序性。  
图示如下：  
...
具体流程就是：父进程先发送一字节的数据，子进程接收数据并且输出信息；子进程发送一字节的数据，父进程接收数据并且输出信息。  
```
#include "kernel/types.h"
#include "kernel/stat.h"
#include "user/user.h"
...
**5.** xargs (moderate)  
**Linux**下的 “**xargs**” 命令常与管道（“**|**”）连起来用，起到的作用是“**从管道中读取输入数据，并进行一定程度的处理（分割、分批），然后传递给后面的命令**”，如下图所示：  
![](https://github.com/2351889401/6.S081-Lab-utilities/blob/main/images/xargs1.png)  
图中管道的输入为“**11m22m33**”，“**-d**”表示后面的字符为分隔符'm'，“**-n**”表示分割之后每一批次传入的参数量为2，将参数传递给最后的“**echo**”命令，所以会有图中的输出。  

本次实验要求完成类似**Linux**下的“**xargs**”的基础作用，能配合管道每次读入一个参数（读到**“\n”**），传递给后面的命令。  
...
```
可以看到，主要就是在不同目录下建立名为“**b**”的文件，然后向其中写入“**hello**”。
  
“**find**”命令的实现是任务 **4** 中实现的。  
...
“**xargs**”从标准输入（这时的标准输入是管道的写端口）每次读入一个参数（就是一个文件路径），后续的行为应该是什么？  
...
```
上述代码中涉及到参数的移位，为什么呢？当时做的时候好像“**argv[]**”数组的申请有点问题，后来就想了个办法：**xargs**的原始命令行参数肯定需要“**argv[]**”数组的，那为什么不直接在原始参数的基础上修改呢？  
